      ...
      <button
        ...
        className={cn(
          'inline-flex items-center justify-center rounded-xl font-medium tracking-wide transition-all active:scale-95 disabled:opacity-50 disabled:pointer-events-none',
          {
            ...
            'bg-primary text-white hover:bg-orange-700 shadow-md shadow-orange-700/10':
              variant === 'primary',
...
export const Card = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div
    className={cn(
      'rounded-2xl border border-border bg-card/60 backdrop-blur-md p-6 shadow-sm cafe-panel',
      className,
    )}
    {...props}
  />
...
export const CardTitle = ({ className, ...props }: React.HTMLAttributes<HTMLHeadingElement>) => (
  <h3
    className={cn('font-serif font-bold text-xl tracking-tight text-foreground', className)}
    {...props}
  />
);
    ...
    <span
      className={cn(
        'inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold uppercase tracking-wider border',
        {
          ...
          'bg-zinc-500/10 border-zinc-500/25 text-zinc-600 dark:text-zinc-400':
            variant === 'neutral',
        },
        className,
      )}
...
export const Table = ({ className, ...props }: React.HTMLAttributes<HTMLTableElement>) => (
  <div className="relative w-full overflow-auto rounded-xl border border-border">
    <table
      className={cn('w-full text-sm text-left border-collapse bg-background/30', className)}
      {...props}
    />
  </div>
);
...
export const TableHeader = ({
  ...
  <thead
    className={cn(
      'bg-muted/80 text-muted-foreground font-semibold text-xs border-b border-border',
      className,
    )}
    {...props}
  />
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-sm animate-fade-in">
      <div className="relative w-full max-w-lg p-6 rounded-2xl border border-border bg-card shadow-2xl cafe-panel mx-4">
        <div className="flex items-center justify-between mb-4 pb-2 border-b border-border">
          ...
          <button
            onClick={onClose}
            className="w-8 h-8 rounded-full hover:bg-muted flex items-center justify-center text-muted-foreground transition-colors font-bold"
          >
            ×
          </button>
        </div>
